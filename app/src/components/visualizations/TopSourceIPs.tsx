...
const TopSourceIPs: React.FC<TopSourceIPsProps> = ({ filter, refreshKey }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const results = await dataService.getTopSourceIPs(filter, 20);
      ...
      console.error('Error loading top source IPs:', err);
  ...
  return (
    <VisualizationWindow title="Top Source IPs" fullscreenChildren={renderContent(700)}>
      {renderContent()}
    </VisualizationWindow>
  );
};
