...
interface TopSourcePortsProps {
  filter: FlowLogFilter;
  refreshKey: number;
}

const TopSourcePorts: React.FC<TopSourcePortsProps> = ({ filter, refreshKey }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const results = await dataService.getTopSourcePorts(filter, 20);
      ...
      console.error('Error loading top source ports:', err);
  ...
  return (
    <VisualizationWindow title="Top Source Ports" fullscreenChildren={renderContent(700)}>
      {renderContent()}
    </VisualizationWindow>
  );
};

export default TopSourcePorts;
