...
const TopTalkers: React.FC<TopTalkersProps> = ({ filter, refreshKey }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const results = await dataService.getTopTalkers(filter, 50);
      ...
      console.error('Error loading top talkers:', err);
          ...
          <tbody>
            {data.map((row, idx) => (
              <tr key={idx}>
                ...
                <td>{formatNumber(row.connection_count)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <VisualizationWindow title="Top Talkers (Conversations)">
      {renderContent()}
    </VisualizationWindow>
  );
};
